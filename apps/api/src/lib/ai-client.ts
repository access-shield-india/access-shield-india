...
/** Local models can take minutes on first download — allow longer timeout. */
function requestTimeoutMs(aiProvider?: string): number {
  return aiProvider === 'local' || aiProvider === 'local-mlx' ? 300_000 : 60_000;
}
